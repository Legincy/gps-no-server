...
func (c *ClusterController) GetAll(ctx *gin.Context) {
	...
	transformedResult := mappers.FromClusterList(clusters, &includeParam)
...
func (c *ClusterController) GetById(ctx *gin.Context) {
	...
	response["payload"] = mappers.FromCluster(cluster, &includeParam)
	ctx.JSON(200, response)
}

func (c *ClusterController) Create(ctx *gin.Context) {
	var clusterDto dtos.ClusterDto
	...
	cluster := mappers.ToCluster(&clusterDto)
	includeParam := ctx.Query("include")
	...
	response["payload"] = mappers.FromCluster(cluster, nil)
...
func (c *ClusterController) Update(ctx *gin.Context) {
	...
	response["payload"] = mappers.FromCluster(updatedCluster, nil)
	ctx.JSON(200, response)
}
